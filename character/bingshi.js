  ...
  potsifeng: {
    ...
    async content(event, trigger, player) {
      ...
      while (cards2.length && targets.length < 2) {
        const num = targets.length || game.countPlayer((current) => current != player) == 1 ? cards2.length : [1, Infinity];
    ...
    subSkill: {
      use: {
        ...
        logTarget: "player",
        async content(event, trigger, player) {
          const suit = get.suit(trigger.card), { targets: [target] } = event, card = target.getExpansions("potsifeng")[0];
